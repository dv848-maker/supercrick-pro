...
const Scorecard = {
  ...
  _renderInnings(inn, battingTeam, bowlingTeam, innBalls, match) {
    ...
    const batTeamNum = inn.battingTeamId === match.team1Id ? 1 : 2;
    const getRoleBadges = (pid, tNum) => {
      const b = [];
      if (match[`team${tNum}Captain`] === pid) b.push('<span class="role-badge-c">C</span>');
      if (match[`team${tNum}VC`] === pid) b.push('<span class="role-badge-vc">VC</span>');
      if (match[`team${tNum}WK`] === pid) b.push('<span class="role-badge-wk">WK</span>');
      return b.join('');
    };
    ...
    for (const pid of battingXI) {
      ...
      const roleBadges = getRoleBadges(pid, batTeamNum);

      html += `<tr>
        <td class="sc-batsman-name">${p ? Utils.shortName(p.name) : '?'} ${roleBadges}</td>
    ...
    const bowlTeamNum = inn.bowlingTeamId === match.team1Id ? 1 : 2;
    for (const pid of bowlingXI) {
      ...
      const bRoleBadges = getRoleBadges(pid, bowlTeamNum);

      html += `<tr>
        <td class="sc-batsman-name">${p ? Utils.shortName(p.name) : '?'} ${bRoleBadges}</td>
        <td>${ov.display}</td>
        <td>${bd.maidens}</td>
        <td>${bd.runs}</td>
